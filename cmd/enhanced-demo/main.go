...
import (
	...
	"passport-pki-demo/internal/userauth"
...
func main() {
	...
	usRegion, usRegionPriv, err := pki.CreateIntermediateCA("USRegion", rootCert, rootPriv)
	if err != nil {
		fmt.Printf("Error creating US region: %v\n", err)
		os.Exit(1)
	}

	europeRegion, europeRegionPriv, err := pki.CreateIntermediateCA("EuropeRegion", rootCert, rootPriv)
	if err != nil {
		fmt.Printf("Error creating Europe region: %v\n", err)
		os.Exit(1)
	}

	asiaRegion, asiaRegionPriv, err := pki.CreateIntermediateCA("AsiaRegion", rootCert, rootPriv)
	if err != nil {
		fmt.Printf("Error creating Asia region: %v\n", err)
		os.Exit(1)
	}

	globalRegion, globalRegionPriv, err := pki.CreateIntermediateCA("GlobalRegion", rootCert, rootPriv)
	if err != nil {
	...
	// Storage certificate
	storageCert, _, err := pki.CreateEndEntityCert("Storage", globalRegion, globalRegionPriv)
	if err != nil {
